...
import { GetProductReviews } from "@/features/reviews";
    ...
    return (
      <div className="flex h-[450px] mb-3 shrink-0 items-center justify-center rounded-md border border-dashed">
        <div className="mx-auto flex max-w-[420px] flex-col items-center justify-center text-center">
          ...
          <p className="mb-4 mt-2 text-sm text-muted-foreground">Sorry, there is no review yet for this product.</p>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-center gap-3">
        {reviews.map((item) => (
          <div key={'product-review-'+item.id} className="flex items-center gap-3">
            <Avatar>
              <AvatarFallback>
                {item.name[0]}
              </AvatarFallback>
            ...
            <div className="w-full">
              <p className="font-bold mb-1 w-full flex items-center">
                {item.name} <Star className="h-3 text-yellow-600"></Star> {item.rating}
              </p>
              <p>{item.review}</p>
            </div>
          </div>
